...
# Other inputs
find_basins = False  # True=create, false=load
...
ERA_resample = (
    True  # True= T-based weight resampling, False= Equal weight resampling
)
...
for B in Basin_names:
    ...
    RGI_dir = basin_info.loc[B]["RGI_dir"]

    p = {
        "basin_shp": join(files, "basin_geojsons", B + ".geojson"),
        ...
        "glob_mask": join(
            files, "globglacfrac", "global_masked_vectorgrid.shp"
        ),
        "Qcatch": join(
            files,
            "HH2018_validation",
            ...
            + "_"
            + GG_rcp.upper()
            + "_Discharge_catchment.dat",
        ),
        "Qm": join(
            files,
            "GG_rasterized_monthly",
            B + "_Qm_" + str(GG_fromyear) + "_" + str(GG_untilyear) + ".nc",
        ),
        "Qd": join(
            files,
            "GG_resampled_daily",
            B + "_Qd_" + str(GG_fromyear) + "_" + str(GG_untilyear) + ".nc",
        ),
        "Qsp": join(
            files,
            "GG_spilling_prevented",
            B + "_Qsp_" + str(GG_fromyear) + "_" + str(GG_untilyear) + ".nc",
        ),
        "Qfinal": join(
            files,
            "GG_final",
        ...
        "Overview": join(
            files,
            "HH2018_validation",
            RGI_dir + "_" + GG_GCM + "_" + GG_rcp.upper() + "_Overview.dat",
        ),
        "ERA_tas": join(
            files,
            "ERA_Interim",
            "pcrglobwb_OBS6_ERA-Interim_reanaly_1_day_tas_1999-2016_"
            + B
            + ".nc",
        ),
        "ERA_pr": join(
            files,
            "ERA_Interim",
            "pcrglobwb_OBS6_ERA-Interim_reanaly_1_day_pr_1999-2016_"
            + B
            + ".nc",
        ),
    ...
    if B == "RHONE":
        ...
        GRDC_no = str(int(basin_info.loc[B]["grdc_no1"]))
...
if find_basins == True:
    for B in Basin_names:
        bas_cont = basin_info.loc[B, "continent"]
        bas_cont_dir = bas_cont + "_bas_dissolved"
        bas_dir = join(
            p["Hydrosheds"], "bas_files", bas_cont_dir, bas_cont_dir + ".shp"
        )
        Basins[B]["shp"] = gp.read_file(
            bas_dir,
            bbox=(
                Basins[B]["center_lon"],
                Basins[B]["center_lat"],
...
for B in Basin_names:
    ...
    Overview = pd.read_csv(
        Basins[B]["p"]["Overview"], delim_whitespace=True, index_col=0
    )
    points = gp.GeoSeries(
        gp.points_from_xy(Overview["lon"].values, Overview["lat"]),
        index=Overview.index,
    )

    Basins[B]["GG_idxs"] = [
        Basins[B]["shp"].geometry.contains(point)[0] for point in points
    ]
...
def load_hybas(Basins, B):
    ...
    main_bas = hybas_full["MAIN_BAS"][
        hybas_full.geometry.contains(
            gp.points_from_xy(
...
if find_sink == True:
    for B in Basin_names:
        ...
        hybas_glac = (
            []
        )  # Create list with subcatchments containing RGI glaciers
        ...
        for i in range(1, 13):
            ...
            if len(unq) > 1:
                ...
                if (
                    un % 10 != 1
                ):  # If the lowest PFAF hybas the sink of the subbasin
                    levelx = hybas[hybas[key] == un]
                    next_down = (
                        levelx.sort_values(by="DIST_MAIN").head(1).NEXT_DOWN
                    )
                    glac_sink = hybas[
                        hybas.HYBAS_ID.values == next_down.values[0]
                    ].centroid
                else:
                    level1 = hybas[hybas[key] == un]
                    glac_sink = (
                        level1.sort_values(by="DIST_MAIN").head(1).centroid
                    )
                break
    ...
    for B in Basin_names:
        x = [basin_info.loc[B, "glac_sink_lon"]]
        y = [basin_info.loc[B, "glac_sink_lat"]]
        Basins[B]["glac_sink"] = gp.GeoSeries(gp.points_from_xy(x, y))

#%% Find GRDC stations
if find_GRDCstations == True:
    # Load GRDC station metadata
    GRDC_meta = pd.read_csv(
        os.path.join(files, "GRDC_Stations.csv"), na_values="n.a.", index_col=0
    )
    ...
    for B in Basin_names:
        ...
        if GR_stats.size == 0:
            ...
            continue
        else:  # Find all stations that fulfill the OBS requirements
            GR_day = GR_stats[
                (GR_stats.d_start <= OBS_maxyear)
                & (GR_stats.d_end >= OBS_minyear)
            ]
            ...
            if GR_day.size == 0:
                ...
                print(
                    ...
                    + " GRDC: "
                    + str(GR_day.shape[0])
                    + " stations loaded with day data"
                )
        Basins[B]["GRDC_stationlist"] = GR_day
    ##### Add GRDC station info to basin_info45min.csv

#%% Find subbasin
if find_GRDCstations == False:
    if find_subbasin == True:
        for B in Basin_names:
            ...
            if B in [
                "OELFUSA",
                "THJORSA",
            ]:  # Station is just downstream of basin borders
                print("Station falls outside of basin, subbasin equals basin")
                Basins[B]["subshp"] = Basins[B]["shp"]
                Basins[B]["subshp"].to_file(
                    Basins[B]["p"]["subbasin_shp"], driver="GeoJSON"
                )
                continue

            hybas = load_hybas(Basins, B)

            station_point = gp.GeoSeries(
                gp.points_from_xy(
                    [Basins[B]["station_lon1"]], [Basins[B]["station_lat1"]]
                )
            )[0]
            ...
            if B in ["COPPER", "OB"]:
                hybas_station = hybas[
                    hybas.HYBAS_ID.values == hybas_station.NEXT_DOWN.values
                ]

            # Select all stations with larger distance to the river sink
            larger_dist = hybas[
                hybas.DIST_MAIN > hybas_station.DIST_MAIN.values[0]
            ]
            t = time.time()
            for i in range(1, 13):
                key = "PFAF_" + str(i)
                ...
                if not np.all(larger_dist[key] == statpfaf):
                    key = "PFAF_" + str(
                        i - 1
                    )  # The last PFAF that all have in common
                    statpfaf = hybas_station[key].values[0]
                    # print (key)
                    ...
                    if np.any(
                        selection.UP_AREA > hybas_station.UP_AREA.values[0]
                    ):
                        ...
                        selection = selection[
                            selection[keyplusone] == statpfafplusone
                        ]
                    ...
                    for j in range(len(selection)):
                        ...
                        while not found:
                            if (
                                next_down.NEXT_DOWN
                                == hybas_station.HYBAS_ID.values[0]
                            ):
                                found = True
                            else:
                                next_down = larger_dist[
                                    larger_dist.HYBAS_ID == next_down.NEXT_DOWN
                                ]
                                if len(next_down) == 0:
...
if find_GRDCstations == False:
    for B in Basin_names:
        # Add a margin of 0.5 and then find the extents
        if B == "COPPER":
            llc = [-149, 60]
            urc = [-137, 66]
        else:
            llc = np.int32(
                np.floor(Basins[B]["subshp"].total_bounds[:2] - 0.5)
            )
...
if find_clone == True:
    for B in Basin_names:
        ...
        command = (
            "gdal_translate -of PCRASTER -projwin "
            + coord2string("llc_lon")
            + " "
            + coord2string("urc_lat")
            + " "
            + coord2string("urc_lon")
            + " "
            + coord2string("llc_lat")
            + " "
            + source
            + " "
            + temp_dest
...
if plot_basin == True:
    for B in Basin_names:
        ...
        if find_GRDCstations == False:  # Plot the selected GRDC station
            Basins[B]["subshp"].plot(
                ax=ax1, color="tab:blue", alpha=0.5, edgecolor="black"
            )
            GRDC = ax1.plot(
                Basins[B]["station_lon1"],
            ...
            # Plot all stations found to match the date requirements
            stations = Basins[B]["GRDC_stationlist"]
            stations.plot(
                x="long", y="lat", kind="scatter", ax=ax1, color="y", s=30
            )
            xtext = 0.1 * (abs(ax1.get_xlim()[1] - ax1.get_xlim()[0]))

            for idx in stations.index:
                xy = np.array(
                    [stations.loc[idx, "long"], stations.loc[idx, "lat"]]
                )
                ax1.annotate(
                    idx,
                    xy,
        ...
        Basins[B]["GG_points"].plot(
            ax=ax1,
            color="red",
            markersize=Basins[B]["GG_area0"] * 2,
            label="Glaciers",
        )
        ...
        if save_fig == True:
            save_at = join(
                run_dir, r"Code\Figures\Basin_maps\subbasins", B + "_map.svg"
            )
...
if find_isout_isglac == True:
    for B in Basin_names:
        ...
        glac_frac = gp.read_file(
            Basins[B]["p"]["globglacfrac"], bbox=bbox
        )  # basin_geo = shape(basin['geometry']).buffer(0.0001) #buffer against invalid shape error
        glob_mask = gp.read_file(
            Basins[B]["p"]["glob_mask"], bbox=bbox
        )  # Global vectorized landmask on the GHM resolution

        print(B + " isout & isglac")
        basin_geo = Basins[B]["subshp"].geometry[0]
        # basin_geo = basin_geo.buffer(-0.001)

        isglac_idx = []  # append all grid cells with glaciers in it
        isout_idx = (
            []
        ...
        isbasin_vec.plot(
            ax=ax1, color="tab:red", alpha=0.7, label="Basin grid"
        )
        isglac_vec.plot(
            ax=ax1, alpha=0.7, color="tab:blue", label="Glacier grid"
        )
        ...
        Basins[B]["subshp"].plot(
            ...
            facecolor="None",
            edgecolor="tab:blue",
            label="Basin shapefile",
        )
        # ax1.legend(['a','b','c','d'])
        ax1.set_title(B.title())

        # Rasterize isglac & isout
        def rasterize_glacout(vector, key):
            vector["bool"] = 1
            llc_lon, llc_lat, urc_lon, urc_lat = [
                Basins[B][i]
                for i in ["llc_lon", "llc_lat", "urc_lon", "urc_lat"]
            ]
            lon360 = 360 * (llc_lon < 0)
            latflip = 1 - 2 * (llc_lat < 0)
            # lon360 = 0
            # latflip=1
            vector.geometry = vector.geometry.affine_transform(
                [1, 0, 0, latflip, lon360, 0]
            )
            res = 0.0833333
            ...
            geo_grid = geo_grid.where(
                ~xr.ufuncs.isnan(geo_grid), 0, drop=False
            )
            return geo_grid
    ...
    for B in Basin_names:
        with xr.open_dataset(
            Basins[B]["p"]["isout"]
        ) as isout_, xr.open_dataset(
            Basins[B]["p"]["isglac"]
        ) as isglac_, xr.open_dataset(
            Basins[B]["p"]["isbasin"]
        ) as isbasin_:
...
for B in Basin_names:
    if Basins[B]["llc_lat"] > 0:
        ...
        Basins[B]["Q_months"] = pd.date_range(
            str(GG_fromyear - 1) + "-09", str(GG_untilyear) + "-10", freq="M"
        )  # .strftime('%Y-%m')
        Basins[B]["Q_days"] = pd.date_range(
            str(GG_fromyear - 1) + "-09-01", str(GG_untilyear) + "-09-30"
        )
    else:
        startdate = str(GG_fromyear - 1) + "-04-01"
        enddate = str(GG_untilyear) + "-03-31"
        Basins[B]["Q_months"] = pd.date_range(
            str(GG_fromyear - 1) + "-03", str(GG_untilyear) + "-04", freq="M"
        )  # .strftime('%Y-%m')
        Basins[B]["Q_days"] = pd.date_range(
            str(GG_fromyear - 1) + "-03-01", str(GG_untilyear) + "-03-31"
        )
...
for B in Basin_names:
    ...
    if (Basins[B]["RGI_dir"] == "RGI17_SouthernAndes") or (
        Basins[B]["RGI_dir"] == "RGI16_LowLatitudes"
    ):
        delimiter = ","
    else:
        delimiter = "   "
    ...
    Q_catch = pd.read_csv(
        ...
        usecols=[
            0,
            *range(
                (GG_fromyear - 1980) * 12, (GG_untilyear + 1 - 1980) * 12 + 1
            ),
        ],
        header=None,
        delimiter=delimiter,
...
if find_Qm == True:
    ...
    def rasterize(month, Basins, B, res=GHM_resolution):
        ...
        geo_grid = make_geocube(
            ...
            rasterize_function=partial(
                rasterize_image, merge_alg=MergeAlg.add
            ),
            fill=0,
        )
        geo_grid["x"] = geo_grid["x"] - lon360
    ...
    for B in Basin_names:
        ...
        print(
            "Monthly rastersum is "
            + str(round((rastersum / pointsum) * 100, 3))
            + " % of pointsum"
        )
    ...
    for B in Basin_names:
        if os.path.isfile(Basins[B]["p"]["Qm"]) == False:
...
if find_Qd == True:
    for B in Basin_names:
        ...
        Qd = Qd.sel(
            time=Basins[B]["daterange"]
        )  # Get rid of extra months at start and finish
        ...
        Q_mlen = [
            m.day for m in Basins[B]["Q_months"]
        ]  # Lengths of months over the daterange
        ...
        if ERA_resample == True:

            ### LOAD ERA-INTERIM DATA
            ERA_full = xr.open_dataset(Basins[B]["p"]["ERA_tas"]).load()
            ERA_full["lon"] = xr.where(
                ERA_full.lon > 180, ERA_full.lon - 360, ERA_full.lon
            )
            ERA_full["lon_bnds"] = xr.where(
                ERA_full.lon_bnds > 180,
                ERA_full.lon_bnds - 360,
                ERA_full.lon_bnds,
            )
            ...
            for (
                ...
                Q_mlen
            ):  # nvm #[1:-1]: #Get rid of extra months at start and finish
                for lats in ERA.lat:
                    for lons in ERA.lon:
                        T = (
                            ERA.isel(time=slice(day, day + D))
                        ...
                        if pd_count == 0:
                            w_15.loc[w_15.time[day : day + D], lats, lons] = (
                                1 / D
                            )  # to make weights sum to 1 and not lose any melt on subzero days
                        else:
                            ...
                            w_15.loc[
                                w_15.time[day : day + D], lats, lons
                            ] = xr.where(
                                positive_days,
                                (1 + weight_factor * (Tp - Tm) / Tm)
                                / pd_count,
                                0,
                            )
                day += D
            ...
            # Upscale to 5 arcmin
            w_05 = w_15.interp_like(
                Qd, method="nearest", kwargs={"fill_value": "extrapolate"}
            )

            if not np.all(
                np.round(w_05.sum(axis=0)) == len(Basins[B]["Q_months"]) - 1
            ):
    ...
    for B in Basin_names:
        ...
        for i in range(Qd.dims["lat"]):
            for j in range(Qd.dims["lon"]):
                lats = Qd.lat[i]
                lons = Qd.lon[j]
                b = Basins[B]["isout"].sel(
                    lon=lons, lat=lats, method="nearest", tolerance=0.05
                )
                success = False
                if b == 1:  # runoff falls outside basin
                    for ii in [-1, 0, 1]:
                        ...
                        for jj in [-1, 0, 1]:
                            ...
                            latss = Qd.lat[i + ii]
                            lonss = Qd.lon[j + jj]
                            c = Basins[B]["isout"].sel(
                                lon=lonss,
                                lat=latss,
                                method="nearest",
                                tolerance=0.01,
                            )
                            isgl = Basins[B]["isglac"].sel(
                                lon=lonss,
                                lat=latss,
                                method="nearest",
                                tolerance=0.01,
                            )
                            isbasin = Basins[B]["isbasin"].sel(
                                lon=lonss,
                                lat=latss,
                                method="nearest",
                                tolerance=0.01,
                            ...
                            if ii == jj == 1:
                                spilled = (
                                    spilled + Qd.R.loc[:, lats, lons].sum()
                                )
                                print(
                                    "Runoff falls out of basin at",
                                    latss.data,
                                    lonss.data,
                                )
                        if success == True:
        ...
        print(
            str(np.round(float(outfrac) * 100, 3))
            + "% of the total runoff would fall outside basin and is thrown back in"
        )
        print(
            str(np.round(float(spillfrac) * 100, 3))
            + "% of the total runoff is spilled"
        )
        ...
        if B == "RHONE":
            ...
            Aletsch_Q.to_netcdf(
                join(files, "Aletsch_Q_" + str(weight_factor) + ".nc")
            )
            Rhoneglac_Q = Qd.R[:, 42, 64]
            Rhoneglac_Q2 = Qd.R[:, 41, 63]
            Rhoneglac_Q.to_netcdf(
                join(files, "Rhoneglac_Q_" + str(weight_factor) + ".nc")
            )
            Rhoneglac_Q2.to_netcdf(
                join(files, "Rhoneglac_Q2_" + str(weight_factor) + ".nc")
            )
...
for B in Basin_names:
    ...
    if include_hg:
        ...
        if B == "RHONE":
            beau_total = pd.read_csv(
                Basins[B]["p"]["Beaucaire"], index_col=0, parse_dates=True
            )
            tarr_total = pd.read_csv(
                Basins[B]["p"]["Tarrascon"], index_col=0, parse_dates=True
            )
            beautarr = pd.concat([beau_total[:"2009"], tarr_total["2010":]])
            beautarr = beautarr.rename(columns={"0": "Q"})
            hg = beautarr[
                str(GG_fromyear - 1) + "-10" : str(GG_untilyear) + "-09"
            ]
            Qfinal["hg"] = (("time"), hg["Q"])
            Qfinal.hg.attrs[
                "long_var_name"
            ] = "Hydrograph of observations downstream"
            Qfinal.hg.attrs["unit"] = "m3/s"
            Qfinal.hg.attrs["source"] = "Hydrobanque France"
            Qfinal.hg.attrs[
                "station"
            ] = "Beaucaire 1980-2009, Tarrascon 2010-2020"
        elif B == "INDUS":
            tarbela = pd.read_csv(
                Basins[B]["p"]["Tarbela"],
                index_col=0,
            ...
            for path in Basins[B]["p"]["GRDC"]:
                ...
                Qfinal.hg.attrs["observation_date_range"] = (
                    dt.datetime.strftime(hg.index[0].date(), "%Y-%m-%d"),
                ...
                Qfinal.hg.attrs["station"] = os.path.basename(path).split("_")[
                    1
                ]
                stations += 1
        Qfinal.attrs["station_coords1"] = (
            Basins[B]["station_lon1"],
            Basins[B]["station_lat1"],
        )
    ...
    for B in Basin_names:
        ...
        Qfinal.to_netcdf(
            join(
                Basins[B]["p"]["nc_out"],
                "_".join(
                    [B, str(GG_fromyear), str(GG_untilyear), run_name, "R.nc"]
                ),
            )
        )
...
if create_worldmap == True:
    ...
    oceanalpha = 0.02
    ax1 = f1.add_axes(projection=ccrs.PlateCarree(), rect=[0, 0, 1, 1])
    ...
    # NEW-ZEALAND subaxes
    ax2 = f1.add_axes(
        projection=ccrs.PlateCarree(), rect=[0.762, 0.00, 0.3, 0.4]
    )
    ...
    ax3 = f1.add_axes(
        projection=ccrs.PlateCarree(), rect=[0.007, 0.519, 0.39, 0.51]
    )
    ...
    # Iceland subaxes
    ax4 = f1.add_axes(
        projection=ccrs.PlateCarree(), rect=[0.470, 0.0, 0.4, 0.3]
    )
